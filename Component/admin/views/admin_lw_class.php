...
class LivingWordAdmin 
{
	public function __construct() {
		global $lwConfig, $wpdb;
    define( 'LIVINGWORD_PLUGINFULLPATH', plugin_dir_path( __FILE__ ));
    $lang = get_locale();
    include_once(LIVINGWORD_PLUGINFULLPATH.'assets/language/livingword-'.$lang.'.php');
    $lwConfig_db = $wpdb->get_results( "SELECT option_name, option_value FROM ".$wpdb->options." WHERE option_name LIKE '%livingword%'" );
    $lwConfig = array(); 
    foreach ( $lwConfig_db as $o ) { 
      $value = maybe_unserialize($o->option_value); 
      $lwConfig[$o->option_name] = $value[$o->option_name]; 
    } 
  }
  ...
  function LWgetPrefOptions(){
    global $bible_version, $bible_plan, $wpdb;
    $bible_version = array(
    ...
    $bible_plan = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'livingword_plans WHERE published=1 ORDER BY id', OBJECT );
  }
...
//    $lang->load( 'com_livingword', JPATH_SITE); 
    include_once(LIVINGWORD_PLUGINFULLPATH.'helpers/lw_version.php');
    $lwversion = new LWVersion();
	  ?>
		<div align="center" class="small">
		  <a href="<?php echo $lwversion->getUrl()?>" target="_blank">LivingWord Plugin - <?php echo $lwversion->getShortCopyright();?></a>
		</div>
	  <?php
	}
  function LWdirToArray($dir) { 
     ...
     { 
        ...
        { 
           ...
           { 
              $result[$value] = $this->LWdirToArray($dir . DIRECTORY_SEPARATOR . $value); 
           } 
           else 
           { 
              if($value != 'index.html' && $value != 'livingword-en_US.php') $result[] = $value; 
           } 
        } 
     } 
  ...
  function matchBookName(&$rmatch){
    global $lwConfig;
    $langdir = strtolower($this->LWgetLang($lwConfig['livingword_bible_version']));
    $lang = get_locale();
    include_once(LIVINGWORD_PLUGINFULLPATH.'assets/language/'.$langdir.'/livingword_biblebooks-'.$lang.'.php'); 
    $rreadingmatch = ucwords(constant($rmatch[1]));
    !$this->checkutf8($lwConfig['livingword_bible_version']) ? $reading_str = htmlentities(html_entity_decode($rreadingmatch)).' '.$rmatch[2] : $reading_str = $rreadingmatch.' '.$rmatch[2];
    return $reading_str;
  }
